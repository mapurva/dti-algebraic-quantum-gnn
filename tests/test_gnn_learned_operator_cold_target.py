...
from data.protein_kmer_operator_features import (
    build_protein_kmer_operator_features
)
...
def main():
    ...
    operator = LearnableSpectralOperator(num_scales=4)

    protein_feats = build_protein_kmer_operator_features(
        proteins, operator
    )
    ...
    optimizer = optim.Adam(
        list(model.parameters()) + list(operator.parameters()),
        lr=1e-3
    )
    ...
    print("\n=== Drug GNN + Learnable Spectral Operator (Cold-Target) ===")
